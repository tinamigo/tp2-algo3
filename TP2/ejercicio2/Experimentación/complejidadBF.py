...
for n in [10 * i for i in range(2,8)]:
	for p in [0.2, 0.4, 0.8]:
		...
		if algoritmo == "coso":
			...
			#Fijate que exp_bellmanFord_time (y los otros algos tmbn) va a tener que imprimir m,tiempo
			#os.system("rm -rf exp_time_tmp_"+algoritmo+_"_bellmanFord.txt")
		elif algoritmo == "dijkstra":
			f=open("Tiempos/complejidades_dijkstra.txt", "a")
			f.write(str(n)+","+str(p)+",")
			f.close()
			os.system("../dijkstra/dijkstra < exp_time_tmp_"+algoritmo+".txt >> Tiempos/complejidades_dijkstra.txt")
		elif algoritmo == "dijkstraDense":
