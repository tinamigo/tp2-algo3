...
for n in [10 * i for i in range(2,10)]:
	for p in [0.2, 0.4, 0.8]:
		#K = i * j * 255
		os.system("python3 generarGrafo.py "+str(n)+" "+str(p))
		if algoritmo == "coso":
			...
			f.write(str(n)+","+str(p)+",")
			f.close()
			print("../bellmanFord/bellmanFord < exp_time_tmp.txt >> Tiempos/complejidades_bellmanFord.txt")
			os.system("../bellmanFord/bellmanFord < exp_time_tmp.txt >> Tiempos/complejidades_bellmanFord.txt")
			#Fijate que exp_bellmanFord_time (y los otros algos tmbn) va a tener que imprimir m,tiempo
			#os.system("rm -rf exp_time_tmp_bellmanFord.txt")
		elif algoritmo == "dijkstra":
			f=open("Tiempos/complejidades_dijkstra.txt", "a")
			f.write(str(n)+","+str(p)+",")
			f.close()
			os.system("../dijkstra/dijkstra < exp_time_tmp.txt >> Tiempos/complejidades_dijkstra.txt")
		elif algoritmo == "dijkstraDense":
			f=open("Tiempos/complejidades_dijkstraDense.txt", "a")
			f.write(str(n)+","+str(p)+",")
			f.close()
			os.system("../dijkstraDense/dijkstraDense < exp_time_tmp.txt >> Tiempos/complejidades_dijkstraDense.txt")
		elif algoritmo == "floyd":
			f=open("Tiempos/complejidades_floyd.txt", "a")
			f.write(str(n)+","+str(p)+",")
			f.close()
			os.system("../floyd/floyd < exp_time_tmp.txt >> Tiempos/complejidades_floyd.txt")
		else:
			os.system("echo LPM")
